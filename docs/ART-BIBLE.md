...
### Current Sandstone slice

The current playable slice shifts the palette from cold industrial teal to warm sandstone: `#9B6B45` floor, `#B98250` walls, `#D0A06A` lit edges, deep umber shadows, turquoise objective markers and amber warning states. The main menu uses the same palette over a darkened hero image so the menu and the 3D field read as one product.
