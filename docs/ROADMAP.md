...
Цель первого прототипа — не кейсы и не три polished maps. Текущий executable blockout называется `Sandstone`; `de_riftfall` ниже остаётся production-кодовым именем следующего server-authoritative map pass. Нужно доказать одну честную 5×5 петлю:
